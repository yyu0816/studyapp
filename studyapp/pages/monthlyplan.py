...
def render_monthly_plan_page() -> None:
    ...
    html_calendar = """
    <div style="display: grid; grid-template-columns: repeat(7, 1fr); border-top: 1px solid #ddd; border-left: 1px solid #ddd; border-radius: 8px; overflow: hidden; background: #fff;">
    """
    ...
    for week in weeks:
        for day in week:
            ...
            bg_color = "#fff" if is_in_range else "#fcfcfc"
            text_color = "#555" if is_in_range else "#bbb"
            ...
            # Show daily tasks in the grid too? The user said "是表情符號+行程名稱要在格子中", I already have fixed_events. 
            # I will also add tasks (進度) if they exist, so the calendar is complete.
            if is_in_range:
